package com.spower.gulimall.seckill.service.impl;
...
import com.spower.gulimall.seckill.to.SeckillSkuRedisTo;
...
import com.spower.gulimall.seckill.feign.CouponFeignService;
import com.spower.gulimall.seckill.feign.ProductFeignService;
import com.spower.gulimall.seckill.interceptor.LoginUserInterceptor;
import com.spower.gulimall.seckill.service.SeckillService;
import com.spower.gulimall.seckill.vo.SeckillSessionWithSkusVo;
import com.spower.gulimall.seckill.vo.SkuInfoVo;
        ...
        if (currentTime >= startTime && currentTime <= endTime) {
            ...
            if (randomCode.equals(key) && killId.equals(skuId)) {
                ...
                if (count > 0 && num <= seckillLimit && count > num) {
                    //4、验证这个人是否已经买过了（幂等性处理）,如果秒杀成功，就去占位。占位名字：userId-sessionId-skuId
                    //SETNX 原子性处理
                    String redisKey = user.getId() + "-" + skuId;
                    //设置自动过期(活动结束时间-当前时间)
                    Long ttl = endTime - currentTime;
                    //setIfAbsent == setNX 是原子的，就可以做到 该用户根据redisKey判断是否买过  买过则返回false 否则true并且占位
